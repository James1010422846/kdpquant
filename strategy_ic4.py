...
def fac(df):
    ...
    df['fac1'] = df.low.rolling(20).min()
    df['fac2'] = (-df.low.rolling(20).min().shift(1))
    df['fac3'] = (-df['turnover'].rolling(20).std().shift(1))
    df['fac4'] = (df['high'].rolling(20).max().shift(1))
    df['fac5'] = (df['close'].rolling(20).kurt().shift(1))
    df['fac6'] = (df['close'].rolling(20).skew().shift(1))
    df['fac7'] = (df['close'].rolling(20).corr().shift(1))
    df['fac1'] = df['fac7']
    ...
    #return df['fac1']
df = pd.DataFrame()
for i in code_list['Tables_in_akshare-price-hfq'][6:]:
    print(i)
    df2 = select(i)
    df2 = df2.dropna()
