...
# 合并两个数据表
df_merge = df300m.merge(df_300, how='inner', on='date')
print(df_merge)
# 将合并数据表的日期列改为时间格式
df_merge['date'] = pd.to_datetime(df_merge['date'], format='%Y-%m-%d')
target = (df_merge['持有基金价值']/df_merge['持有份额']).pct_change()
# 将索引设置为日期列
target.index = df_merge['date']
# 基准策略收益率Series，计算方法相同
base = df_merge['基金净值'].pct_change()
...
plt.plot(df300m.index, df300m['持有基金价值'])
