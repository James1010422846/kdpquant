...
x = 20
for i in code_list['Tables_in_akshare-price-hfq'][6:x]:
    print(i)
    df2 = select(i)
    if len(df2.close)<250:
        pass
    else:
        df2 = df2.dropna()
        df2 = df2.replace([np.inf, -np.inf], 0)
        df2 = Strategy(df2)[['date','nav']]
        df = pd.merge(df,df2,on='date')
df['nav_last'] = df.iloc[:,-len(code_list['Tables_in_akshare-price-hfq'][6:x]):].mean(axis=1)
