...
# 策略函数
def Strategy(fm):
    fm['upper'], fm['middle'], fm['lower']  = ta.BBANDS(fm.close, timeperiod=20,nbdevup=2,
nbdevdn=2,matype=0)
    ...
    #     # 当前空仓、金叉，买进。
    #     if (fm.close[i - 2] <fm.upper[i-2]) &(fm.close[i - 1] > fm.upper[i-1])  & (fm.position[i] == 0):
    #         fm.iloc[i, -1] = 1
    #         fm.iloc[i + 1, -2] = 1
    #     # 当前持仓、死叉，卖出。
    #     elif (fm.close[i - 2] >fm.lower[i-2]) &(fm.close[i - 1] < fm.lower[i-1])  & (fm.position[i] == 1):
    #         fm.iloc[i, -1] = -1
    #         fm.iloc[i + 1, -2] = 0
    #
    #         # 其他情况，仓位不变。
    ...
    for i in range(12, len(fm) - 1):
        ...
        if (fm.close[i - 2] > fm.lower[i - 2]) & (fm.close[i - 1] < fm.lower[i - 1]) & (fm.position[i] == 0):
            fm.iloc[i, -1] = 1
            fm.iloc[i + 1, -2] = 1
        # 当前持仓、死叉，卖出。
        elif (fm.close[i - 2] <fm.upper[i-2]) &(fm.close[i - 1] > fm.upper[i-1])   & (fm.position[i] == 1):
            fm.iloc[i, -1] = -1
            fm.iloc[i + 1, -2] = 0
