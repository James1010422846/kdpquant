...
def fac(df):
    df['fac1'] = ta.OBV(df.close,df.volume)
    #print(ta.OBV(df.close,df.volume))
    print(mad(df['fac1']))
    print(stand(mad(df['fac1'])))
    return stand(mad(ta.OBV(df.close,df.volume)))
df = pd.DataFrame()
for i in code_list['Tables_in_akshare-price-hfq'][6:7]:
    df2 = select(i)
    df2 = df2.dropna()
...
ic_analysis.ic_analysis(assets[['OBV']], close)